...
import { Box, Container, Typography, Breadcrumbs, Link as MuiLink, Chip, Avatar, Paper, Button } from "@mui/material";
        ...
        openModal({
            ...
            actions: [
                ...
                },
                {
                    ...
                    onClick: async () => {
                        try {
                            await deleteTopic(topic.id, Number(session.user.id));
                            showToast("success", "Topic deleted successfully!");
                            router.push(`/forum/categories/${category.id}/${category.slug}`);
                        } catch (error) {
                            showToast("error", error instanceof Error ? error.message : "Failed to delete topic");
                        }
                    },
                    type: "submit",
                    color: "secondary",

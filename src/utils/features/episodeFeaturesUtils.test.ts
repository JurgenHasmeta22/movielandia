...
describe('Episode Features Utils', () => {
  ...
  describe('onBookmarkEpisode', () => {
    ...
    it('should handle unknown errors', async () => {
      ...
      expect(addFavoriteEpisodeToUser).toHaveBeenCalledWith(1, mockEpisode.id);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while adding the episode to favorites.');
      expect(console.error).toHaveBeenCalled();
    });
  ...
  describe('onRemoveBookmarkEpisode', () => {
    it('should remove episode from favorites and show success toast', async () => {
      ...
      expect(removeFavoriteEpisodeToUser).toHaveBeenCalledWith(1, mockEpisode.id, `/episodes/${mockEpisode.title}`);
    ...
    it('should handle unknown errors', async () => {
      ...
      expect(removeFavoriteEpisodeToUser).toHaveBeenCalledWith(1, mockEpisode.id, `/episodes/${mockEpisode.title}`);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while removing the episode from favorites.');
      expect(console.error).toHaveBeenCalled();
    });

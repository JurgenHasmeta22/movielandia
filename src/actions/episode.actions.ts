		...
		if (episode) {
			...
			if (userId) {
				...
				const existingFavorite =
					await prisma.userEpisodeFavorite.findFirst({
						where: {
							AND: [{ userId }, { episodeId: episode.id }],
						},
					});
...
export async function getLatestEpisodes(
	...
	const episodeRatingsMap: RatingsMap = episodeRatings.reduce(
		(map, rating) => {
...
export async function updateEpisodeById(
	episodeParam: Prisma.EpisodeUpdateInput,
	id: string,
): Promise<Episode | null> {
...
export async function addEpisode(
	episodeParam: Prisma.EpisodeUncheckedCreateInput,
): Promise<Episode | null> {

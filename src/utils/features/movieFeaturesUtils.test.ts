...
describe("Movie Features Utils", () => {
	...
	describe("onBookmarkMovie", () => {
		it("should add movie to favorites and show success toast", async () => {
			...
			expect(addFavoriteMovieToUser).toHaveBeenCalledWith(
				1,
				mockMovie.id,
			);
		...
		it("should handle unknown errors", async () => {
			vi.mocked(addFavoriteMovieToUser).mockRejectedValueOnce(
				"Unknown error",
			);
	...
	describe("onRemoveBookmarkMovie", () => {
		it("should remove movie from favorites and show success toast", async () => {
			...
			expect(removeFavoriteMovieToUser).toHaveBeenCalledWith(
				1,
				mockMovie.id,
				`/movies/${mockMovie.title}`,
			);
			expect(showToast).toHaveBeenCalledWith(
				"success",
		...
		it("should handle errors and show error toast", async () => {
			const testError = new Error("Test error");
			vi.mocked(removeFavoriteMovieToUser).mockRejectedValueOnce(
				testError,
			);

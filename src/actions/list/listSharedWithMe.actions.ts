...
export async function getSharedLists(userId: number, params: SharedListsParams = {}) {
    const { page = 1, perPage = 12, sortBy = "createdAt", ascOrDesc = "desc" } = params;
    ...
    try {
        ...
        return {
            ...
            total: 0,
            success: false,
            error: error instanceof Error ? error.message : "Failed to fetch shared lists",
        };
    }
}
...
export async function getRecentSharedLists(userId: number, limit: number = 5) {
    try {
        ...
        return {
            items: [],
            success: false,
            error: error instanceof Error ? error.message : "Failed to fetch recent shared lists",
        };
    }
}

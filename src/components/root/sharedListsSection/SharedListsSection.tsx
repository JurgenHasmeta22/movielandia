...
export default function SharedListsSection({ userId, userName }: SharedListsSectionProps) {
            ...
            >
                {sharedLists.map((list) => (
                    <Box key={list.id} sx={{ flexShrink: 0 }}>
                        <ListCard list={list} username={userName} userId={userId} isSharedView={true} />
                    </Box>
                ))}
            </Stack>
        </Box>

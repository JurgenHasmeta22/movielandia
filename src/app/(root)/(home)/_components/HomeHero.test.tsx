...
describe("HomeHeroSection", () => {
	...
	it("renders the component correctly", () => {
		...
		expect(
			screen.getByText("Your Gateway to the World of Cinema!"),
		).toBeDefined();

		// Check description text
		expect(
			screen.getByText(
				/Explore the latest blockbusters and timeless classics/i,
			),
		).toBeDefined();
		...
		expect(backgroundImage.getAttribute("data-src")).toBe(
			"/images/backgrounds/netflix.png",
		);
		expect(backgroundImage.getAttribute("data-alt")).toBe(
			"Background Image",
		);
	});
	...
	it("contains the feature text", () => {
		renderWithTheme(<HomeHeroSection />);
		expect(
			screen.getByText("Featuring thousands of titles across all genres"),
		).toBeDefined();
	});
	...
	it("has proper button styling and content", () => {
		...
		const exploreMoviesButton = screen
			.getByText("Explore Movies")
			.closest("a");
		const browseSeriesButton = screen
			.getByText("Browse Series")
			.closest("a");

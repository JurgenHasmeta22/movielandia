		...
		if (season) {
			...
			if (userId) {
				...
				const existingFavorite =
					await prisma.userSeasonFavorite.findFirst({
						where: {
							AND: [{ userId }, { seasonId: season.id }],
						},
					});
...
export async function getLatestSeasons(
	serieId: number,
): Promise<Season[] | null> {
	"use cache";
...
export async function updateSeasonById(
	seasonParam: Prisma.SeasonUpdateInput,
	id: string,
): Promise<Season | null> {
...
export async function addSeason(
	seasonParam: Prisma.SeasonCreateInput,
): Promise<Season | null> {
...
export async function searchSeasonsByTitle(
	title: string,
	queryParams: any,
	userId?: number,
): Promise<any | null> {

...
export default function SocialSection({
	...
	const handleMessageUser = () => {
		if (
			...
			userInPage.isFollowedStatus !== "accepted"
		)
			return;
		...
		<Box>
			{userLoggedIn && userLoggedIn.id !== userInPage.id && (
				<Stack direction="row" spacing={1} alignItems="center">
					<Button
						variant={
							userInPage.isFollowed ? "outlined" : "contained"
						}
						startIcon={
							isPending ? (
			...
			{userLoggedIn &&
				...
				userPendingFollowers.items.length > 0 && (
					<Box sx={{ mt: 2, maxWidth: "100%" }}>
						<Link
							href={`/users/${userInPage.id}/${userInPage.userName}/followersRequests`}
						>
							<Button
								variant="outlined"
								sx={{
							...
							>
								Follow Requests (
								{userPendingFollowers.items.length})
							</Button>
						</Link>
					</Box>
				)}

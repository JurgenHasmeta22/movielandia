...
describe("SearchResultCard Component", () => {
	...
	it("renders a season card correctly with route params", () => {
		...
		expect(link.getAttribute("href")).toBe(
			"/series/10/Parent-Serie/seasons/6/Test-Season",
		);
	});
	...
	it("renders an episode card correctly with route params", () => {
		...
		const link = screen.getByRole("link");
		expect(link.getAttribute("href")).toBe(
			"/series/10/Parent-Serie/seasons/20/Season-One/episodes/7/Test-Episode",
		);
	});
	...
	it("handles missing description", () => {
		...
		expect(
			screen.queryByText("This is a test movie description"),
		).toBeNull();
	});
	...
	it("handles different path types for actor cards", () => {
		// Create a modified actor data with title for movie path
		const actorWithTitle = { ...actorData, title: "Test Actor" };

		// Test with movies path
		renderSearchResultCard(actorWithTitle, "actor", "movies");
		...
		link.parentElement?.remove();
		// Add title property for series path
		const actorWithTitleForSeries = { ...actorData, title: "Test Actor" };
		renderSearchResultCard(actorWithTitleForSeries, "actor", null);
		link = screen.getByRole("link");
		expect(link.getAttribute("href")).toBe("/series/3/Test-Actor");
	});

	it("handles different path types for crew cards", () => {
		// Create a modified crew data with title for movie path
		const crewWithTitle = { ...crewData, title: "Test Crew" };

		// Test with movies path
		renderSearchResultCard(crewWithTitle, "crew", "movies");
		let link = screen.getByRole("link");
		...
		link.parentElement?.remove();
		// Add title property for series path
		const crewWithTitleForSeries = { ...crewData, title: "Test Crew" };
		renderSearchResultCard(crewWithTitleForSeries, "crew", null);
		link = screen.getByRole("link");

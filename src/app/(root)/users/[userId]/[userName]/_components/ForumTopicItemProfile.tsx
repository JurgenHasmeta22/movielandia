        ...
        >
            ...
            <Box sx={{ mb: 1.5 }}>
                <Link href={categoryLink} style={{ textDecoration: "none" }}>
                    <Chip label={topic.category.name} size="small" color="primary" sx={{ mr: 1, mb: 1 }} clickable />
                </Link>
                {topic.tags.slice(0, 3).map((tag) => (
                    <Chip key={tag.id} label={tag.name} size="small" variant="outlined" sx={{ mr: 1, mb: 1 }} />
                ))}
                {topic.tags.length > 3 && (
                    <Chip label={`+${topic.tags.length - 3} more`} size="small" variant="outlined" sx={{ mb: 1 }} />
                )}
            </Box>

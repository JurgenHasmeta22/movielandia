...
export default async function Page(props: PageProps) {
	...
	if (list.contentType) {
		switch (list.contentType) {
			case "movie": {
				const { items, total } = await getListMovies(
					listId,
					currentUserId,
					page,
					itemsPerPage,
			...
			case "serie": {
				const { items, total } = await getListSeries(
					listId,
					currentUserId,
					page,
					itemsPerPage,
			...
			case "season": {
				const { items, total } = await getListSeasons(
					listId,
					currentUserId,
					page,
					itemsPerPage,
			...
			case "episode": {
				const { items, total } = await getListEpisodes(
					listId,
					currentUserId,
					page,
					itemsPerPage,
			...
			case "actor": {
				const { items, total } = await getListActors(
					listId,
					currentUserId,
					page,
					itemsPerPage,
			...
			case "crew": {
				const { items, total } = await getListCrew(
					listId,
					currentUserId,
					page,
					itemsPerPage,
		...
		const detectedType = await detectAndUpdateListContentType(
			listId,
			currentUserId,
		);

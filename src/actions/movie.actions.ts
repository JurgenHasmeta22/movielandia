...
import { Movie, Prisma } from "../../prisma/generated/prisma/client";
...
export async function getRelatedMovies(
	...
	const movies = await Promise.all(
		relatedMovies.map(async (movie) => {
			...
			const ratingsInfo = ratingsMap[movie.id] || {
				averageRating: 0,
				totalReviews: 0,
			};
...
export async function updateMovieById(
	movieParam: Prisma.MovieUpdateInput,
	id: string,
): Promise<Movie | null> {
...
export async function addMovie(
	movieParam: Prisma.MovieCreateInput,
): Promise<Movie | null> {

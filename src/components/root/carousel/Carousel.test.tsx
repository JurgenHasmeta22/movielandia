...
vi.mock("next/image", () => ({
	default: ({ src, alt, height, width, priority, style }: any) => (
		// Using div instead of img to avoid ESLint warnings
		<div
			data-src={src}
			data-alt={alt}
			data-height={height}
	...
	return {
		default: ({ children, ...props }: any) => {
			...
			const serializableProps = {
				...
				fade: props.fade,
				// Omit nextArrow, prevArrow, and other React components
...
describe("Carousel Component", () => {
	...
	it("displays item descriptions", () => {
		...
		expect(
			screen.getByText("This is another test movie description"),
		).toBeDefined();
		expect(
			screen.getByText("This is a test actor description"),
		).toBeDefined();
	});

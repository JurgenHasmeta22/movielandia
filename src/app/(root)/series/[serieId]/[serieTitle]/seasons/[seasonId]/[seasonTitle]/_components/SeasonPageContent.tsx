	...
	async function onUpvoteSeason(
		...
		try {
			...
			if (error instanceof Error) {
				...
				showToast(
					"error",
					"An unexpected error occurred while upvoting the season.",
				);
			}
		}
	}

	async function onDownVoteSeason(
		seasonReviewId: number,
		...
		try {
			...
			if (error instanceof Error) {
				...
				showToast(
					"error",
					"An unexpected error occurred while downvoting the season.",
				);
			}
		}
	}
	...
	return (
		<Stack flexDirection={"column"} rowGap={4}>
			<DetailsPageCard
				...
				onRemoveBookmark={() =>
					onRemoveBookmarkSeason(session!, season)
				}
				onGoBack={() => {
					...
					const newPath =
						seasonsIndex !== -1
							? currentPath.slice(0, seasonsIndex)
							: currentPath;

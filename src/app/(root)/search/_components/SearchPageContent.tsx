...
export default async function SearchPageContent({
	...
	const shouldFetchData = (type: string) =>
		selectedFilters.includes("all") || selectedFilters.includes(type);
	...
	if (shouldFetchData("movies")) {
		...
		const moviesData = await searchMoviesByTitle(
			term,
			queryParamsMovies,
	...
	if (shouldFetchData("series")) {
		...
		const seriesData = await searchSeriesByTitle(
			term,
			queryParamsSeries,
			Number(session?.user?.id),
		);
	...
	if (shouldFetchData("actors")) {
		...
		const actorsData = await searchActorsByTitle(
			term,
			queryParamsActors,
			Number(session?.user?.id),
		);
	...
	if (shouldFetchData("crew")) {
		...
		const crewsData = await searchCrewMembersByFullname(
			term,
			queryParamsCrews,
			Number(session?.user?.id),
		);
	...
	if (shouldFetchData("episodes")) {
		...
		const episodesData = await searchEpisodesByTitle(
			term,
			queryParamsEpisodes,
			Number(session?.user?.id),
		);
	...
	if (shouldFetchData("seasons")) {
		...
		const seasonsData = await searchSeasonsByTitle(
			term,
			queryParamsSeasons,
			Number(session?.user?.id),
		);
			...
			>
				...
				{shouldShowSection("seasons") &&
					(!term || seasonsCount > 0) && (
						<SearchList
							title={
								term ? `Seasons matching "${term}"` : "Seasons"
							}
							data={seasons}
							count={seasonsCount}
							sortBy={searchParams?.seasonsSortBy ?? ""}

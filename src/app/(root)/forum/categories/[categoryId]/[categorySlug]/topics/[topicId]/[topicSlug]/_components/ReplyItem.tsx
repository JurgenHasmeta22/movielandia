...
import {
	...
	deleteReply,
	upvoteReply,
} from "@/actions/forum/forumReply.actions";
	...
	function handleCancelEditing() {
		if (editContent !== originalContent) {
			openModal({
				...
				subTitle:
					"Are you sure you want to discard changes on this reply?",
				actions: [
					{
						label: CONSTANTS.MODAL__DELETE__NO,
						onClick: () => {},
				...
				<>
					...
					>
						...
						<Box sx={{ display: "flex", gap: 1 }}>
							...
							{userLoggedIn &&
								Number(userLoggedIn.id) === reply.user.id &&
								!topicLocked && (
									<Box sx={{ display: "flex", gap: 1 }}>
										<Button
											variant="outlined"

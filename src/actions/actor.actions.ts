...
import { Actor, Prisma } from "../../prisma/generated/prisma/client";
		...
		actors.map(async (actor) => {
			...
			if (userId) {
				const existingFavorite =
					await prisma.userActorFavorite.findFirst({
						where: {
			...
			const ratingsInfo = actorRatingsMap[actor.id] || {
				averageRating: 0,
				totalReviews: 0,
			};
...
export async function updateActorById(
	actorParam: Prisma.ActorUpdateInput,
	id: string,
): Promise<Actor | null> {
...
export async function addActor(
	actorParam: Prisma.ActorCreateInput,
): Promise<Actor | null> {
	try {
...
export async function searchActorsByTitle(
	fullname: string,
	queryParams: any,
	userId?: number,
): Promise<any | null> {

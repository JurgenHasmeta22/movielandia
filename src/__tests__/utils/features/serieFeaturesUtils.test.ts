...
describe('Serie Features Utils', () => {
  ...
  describe('onBookmarkSerie', () => {
    it('should add serie to favorites and show success toast', async () => {
      ...
      expect(addFavoriteSerieToUser).toHaveBeenCalledWith(1, mockSerie.id);
      expect(showToast).toHaveBeenCalledWith('success', 'Serie added to favorites!');
    });
    ...
    it('should handle unknown errors', async () => {
      vi.mocked(addFavoriteSerieToUser).mockRejectedValueOnce('Unknown error');

      await onBookmarkSerie(mockSession, mockSerie);

      expect(addFavoriteSerieToUser).toHaveBeenCalledWith(1, mockSerie.id);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while adding the serie to favorites.');
      expect(console.error).toHaveBeenCalled();
    });
  ...
  describe('onRemoveBookmarkSerie', () => {
    it('should remove serie from favorites and show success toast', async () => {
      ...
      expect(removeFavoriteSerieToUser).toHaveBeenCalledWith(1, mockSerie.id, `/series/${mockSerie.title}`);
      expect(showToast).toHaveBeenCalledWith('success', 'Serie removed from favorites!');
    });

    it('should handle errors and show error toast', async () => {
      const testError = new Error('Test error');
      vi.mocked(removeFavoriteSerieToUser).mockRejectedValueOnce(testError);
    ...
    it('should handle unknown errors', async () => {
      vi.mocked(removeFavoriteSerieToUser).mockRejectedValueOnce('Unknown error');

      await onRemoveBookmarkSerie(mockSession, mockSerie);

      expect(removeFavoriteSerieToUser).toHaveBeenCalledWith(1, mockSerie.id, `/series/${mockSerie.title}`);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while removing the serie from favorites.');
      expect(console.error).toHaveBeenCalled();
    });

...
describe('Crew Features Utils', () => {
  ...
  describe('onBookmarkCrew', () => {
    it('should add crew to favorites and show success toast', async () => {
      ...
      expect(showToast).toHaveBeenCalledWith('success', 'Crew added to favorites!');
    });
    ...
    it('should handle unknown errors', async () => {
      vi.mocked(addFavoriteCrewToUser).mockRejectedValueOnce('Unknown error');
      
      await onBookmarkCrew(mockSession, mockCrew);
      
      expect(addFavoriteCrewToUser).toHaveBeenCalledWith(1, mockCrew.id);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while adding the crew to favorites.');
      expect(console.error).toHaveBeenCalled();
    });
  ...
  describe('onRemoveBookmarkCrew', () => {
    it('should remove crew from favorites and show success toast', async () => {
      ...
      expect(removeFavoriteCrewToUser).toHaveBeenCalledWith(1, mockCrew.id);
      expect(showToast).toHaveBeenCalledWith('success', 'Crew removed from favorites!');
    });

    it('should handle errors and show error toast', async () => {
      const testError = new Error('Test error');
      vi.mocked(removeFavoriteCrewToUser).mockRejectedValueOnce(testError);
    ...
    it('should handle unknown errors', async () => {
      vi.mocked(removeFavoriteCrewToUser).mockRejectedValueOnce('Unknown error');
      
      await onRemoveBookmarkCrew(mockSession, mockCrew);
      
      expect(removeFavoriteCrewToUser).toHaveBeenCalledWith(1, mockCrew.id);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while removing the crew from favorites.');
      expect(console.error).toHaveBeenCalled();
    });

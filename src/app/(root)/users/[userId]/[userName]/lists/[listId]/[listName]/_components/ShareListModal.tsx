            ...
            searchUsers(debouncedSearchQuery, userId)
                .then((results) => {
                    const filteredResults = results.filter(
                        (user: {
                            id: number;
                            avatar: { id: number; photoSrc: string; userId: number } | null;
                            userName: string;
                            email: string;
                        }) => !currentSharedUsers.some((sharedUser) => sharedUser.id === user.id),
                    );
    ...
    const handleUpdatePermission = (targetUserId: number, canEdit: boolean, userName: string) => {
        startTransition(async () => {
            try {
                ...
                setCurrentSharedUsers(
                    currentSharedUsers.map((user) => (user.id === targetUserId ? { ...user, canEdit } : user)),
                );
        ...
        >
            ...
            >
                <Box sx={{ display: "flex", alignItems: "center" }}>
                    <ShareIcon sx={{ mr: 1.5, color: "#fff", fontSize: "1.2rem" }} />
                    ...
                    {selectedUser ? (
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                <Chip
                                    avatar={<Avatar src={selectedUser.avatar?.photoSrc} alt={selectedUser.userName} />}
                            ...
                            >
                                {isPending ? <CircularProgress size={24} /> : "Share with User"}
                            </Button>
                        </Box>
                    ) : (
                        <TextField
                            ...
                            <ListItem
                                ...
                                secondaryAction={
                                    <Box>
                                        ...
                                        >
                                            {user.canEdit ? <EditIcon /> : <VisibilityIcon />}
                                        </IconButton>
                                        <IconButton
                                            edge="end"
                                            aria-label="delete"
            ...
            <DialogActions sx={{ px: 3, py: 2, bgcolor: "#2c3347", borderTop: "1px solid rgba(255, 255, 255, 0.1)" }}>
                <Button
                    onClick={onClose}
                    variant="outlined"
                    sx={{

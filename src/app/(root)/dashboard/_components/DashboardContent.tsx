...
import { Box, Container, Typography, Paper, Grid, Button, Stack } from "@mui/material";
...
export default function DashboardContent({ userId, userName }: DashboardContentProps) {
    ...
    const dashboardItems = [
        {
            title: "My Lists",
            description: "Create and manage your custom lists of movies, TV shows, and more.",
            icon: <PlaylistAddIcon sx={{ fontSize: 40, color: "primary.main" }} />,
            action: () => router.push(`/users/${userId}/${userName}/lists`),
            buttonText: "View Lists",
        },
        {
            title: "My Reviews",
            description: "See all your reviews and ratings for movies and TV shows.",
            icon: <RateReviewIcon sx={{ fontSize: 40, color: "primary.main" }} />,
            action: () => router.push(`/users/${userId}/${userName}?maintab=reviews&subtab=movies`),
            buttonText: "View Reviews",
        },
        {
            title: "My Bookmarks",
            description: "Access your bookmarked movies, TV shows, actors, and more.",
            icon: <BookmarkIcon sx={{ fontSize: 40, color: "primary.main" }} />,
            action: () => router.push(`/users/${userId}/${userName}?maintab=bookmarks&subtab=movies`),

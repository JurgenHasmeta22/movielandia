		...
		crew.map(async (crewMember) => {
			...
			if (userId) {
				const existingFavorite =
					await prisma.userCrewFavorite.findFirst({
						where: {
			...
			const ratingsInfo = crewRatingsMap[crewMember.id] || {
				averageRating: 0,
				totalReviews: 0,
			};
...
export async function getCrewMemberByFullname(
	crewFullname: string,
	queryParams: any,
): Promise<Crew | any | null> {
...
export async function updateCrewMemberById(
	crewParam: Prisma.CrewUpdateInput,
	id: string,
): Promise<Crew | null> {
...
export async function searchCrewMembersByFullname(
	...
	const crewsFinal = await Promise.all(
		crews.map(async (crew) => {
			...
			const ratingsInfo = crewRatingsMap[crew.id] || {
				averageRating: 0,
				totalReviews: 0,
			};

...
export default function UserPageContent({
	...
	const getCurrentSubTabIndex = () => {
		...
		const subTabIndex = currentSubTabs.findIndex(
			(tab) =>
				...
				subTab.toLowerCase().replace(/\s+/g, ""),
		);
	...
	return (
		...
		>
			<Paper
				elevation={3}
				sx={{ width: "100%", p: { xs: 3, sm: 4 }, maxWidth: 800 }}
			>
				<Stack
					direction="column"
					spacing={{ xs: 3, sm: 4 }}
			...
			{!canViewProfile &&
				userLoggedIn &&
				userLoggedIn.id !== userInPage.id && <PrivateProfileMessage />}
			{canViewProfile && (
				<>
					<Box
						sx={{
					...
					>
						...
						>
							{userInPage.userName}&apos;s{" "}
							{mainTabs[getCurrentMainTabIndex()].label}
						</Typography>
						<Typography
							variant="body2"
						...
						>
							{getMainTabDescription(
								mainTabs[getCurrentMainTabIndex()].param,
							)}
						</Typography>
						<Divider sx={{ mt: 2 }} />
					</Box>
					...
					>
						<TabContent
							type={
								subTabs[mainTab as keyof typeof subTabs][
									getCurrentSubTabIndex()
								]
							}
							userLoggedIn={userLoggedIn}
							userInPage={userInPage}

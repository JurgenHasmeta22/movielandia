...
async function generateMovies(count: number, startId: number): Promise<void> {
	console.log(
		`Generating ${count} new movies starting from ID ${startId + 1}...`,
	);
	...
	for (let i = 0; i < movieBatch.length; i++) {
		...
		const selectedGenreIds = faker.helpers.arrayElements(
			genreIds,
			genreCount,
		);

		await Promise.all(
			selectedGenreIds.map((genreId: number) =>
...
async function generateActors(count: number, startId: number): Promise<void> {
	console.log(
		`Generating ${count} new actors starting from ID ${startId + 1}...`,
	);
	...
	for (let i = 0; i < count; i++) {
		...
		const actor = {
			...
			debut: faker.date
				.between({ from: "1980-01-01", to: "2020-01-01" })
				.getFullYear()
				.toString(),
		};
...
async function generateCrew(count: number, startId: number): Promise<void> {
	console.log(
		`Generating ${count} new crew members starting from ID ${startId + 1}...`,
	);
...
async function generateSeries(
	...
	console.log(
		`Generating ${count} new series starting from ID ${startSeriesId + 1}...`,
	);
	...
	console.log(
		`Successfully generated ${count} new series with seasons and episodes.`,
	);
}
...
async function generateRelationships(): Promise<void> {
	...
	for (const movie of movies) {
		...
		for (const actor of selectedActors) {
			...
			if (existingMovieActorRelationships.has(relationshipKey)) {
				console.log(
					`Skipping duplicate castMovie relationship: Movie ${movie.id} - Actor ${actor.id}`,
				);
				continue;
			}
	...
	console.log(
		`Found ${existingRelationships.size} existing castSerie relationships`,
	);
	...
	for (const serie of series) {
		...
		console.log(
			`Processing serie ${serie.id} with ${selectedActors.length} actors`,
		);
		...
		for (const actor of selectedActors) {
			...
			if (existingRelationships.has(relationshipKey)) {
				console.log(
					`Skipping duplicate castSerie relationship: Serie ${serie.id} - Actor ${actor.id}`,
				);
				continue;
			}
			...
			if (existingRelation) {
				console.log(
					`Found existing castSerie in DB: Serie ${serie.id} - Actor ${actor.id}`,
				);
			...
			try {
				...
				console.log(
					`Created castSerie: ID ${castSerieId}, Serie ${serie.id} - Actor ${actor.id}`,
				);
			} catch (error: any) {
				if (error.code === "P2002") {
					console.log(
	...
	for (const movie of movies) {
		...
		for (const crewMember of selectedCrew) {
			...
			if (existingMovieCrewRelationships.has(relationshipKey)) {
				console.log(
					`Skipping duplicate crewMovie relationship: Movie ${movie.id} - Crew ${crewMember.id}`,
				);
				continue;
			}
	...
	console.log(
		`Found ${existingSerieCrewRelationships.size} existing crewSerie relationships`,
	);
	...
	for (const serie of series) {
		...
		const selectedCrew = faker.helpers.arrayElements(crew, crewCount);
		console.log(
			`Processing serie ${serie.id} with ${selectedCrew.length} crew members`,
		);
		...
		for (const crewMember of selectedCrew) {
			...
			if (existingSerieCrewRelationships.has(relationshipKey)) {
				console.log(
					`Skipping duplicate crewSerie relationship: Serie ${serie.id} - Crew ${crewMember.id}`,
				);
				continue;
			}
			...
			if (existingRelation) {
				console.log(
					`Found existing crewSerie in DB: Serie ${serie.id} - Crew ${crewMember.id}`,
				);
			...
			try {
				...
				console.log(
					`Created crewSerie: ID ${crewSerieId}, Serie ${serie.id} - Crew ${crewMember.id}`,
				);
			} catch (error: any) {
				if (error.code === "P2002") {
					console.log(
...
async function generateReviewsAndRatings(): Promise<void> {
	...
	if (users.length === 0) {
		console.log(
			"No users found to create reviews and ratings. Skipping this step.",
		);
		return;
	}
	...
	for (const movie of movies) {
		...
		for (const user of raters) {
			try {
				...
				if (error.code === "P2002") {
					console.log(
						`Skipping duplicate movie rating: Movie ${movie.id} - User ${user.id}`,
					);
				} else {
					throw error;
				}
			...
			if (Math.random() > 0.6) {
				try {
					...
					if (error.code === "P2002") {
						console.log(
							`Skipping duplicate movie favorite: Movie ${movie.id} - User ${user.id}`,
						);
					} else {
						throw error;
					}
	...
	for (const serie of series) {
		...
		for (const user of raters) {
			try {
				...
				if (error.code === "P2002") {
					console.log(
						`Skipping duplicate serie rating: Serie ${serie.id} - User ${user.id}`,
					);
				} else {
					throw error;
				}
			...
			if (Math.random() > 0.6) {
				try {
					...
					if (error.code === "P2002") {
						console.log(
							`Skipping duplicate serie favorite: Serie ${serie.id} - User ${user.id}`,
						);
					} else {
						throw error;
					}
	...
	for (const actor of actors) {
		...
		for (const user of reviewers) {
			...
			try {
				...
				console.log(
					`Skipping duplicate actor review for actor ${actor.id} by user ${user.id}`,
				);
			}
		}
		...
		for (const user of raters) {
			try {
				...
				console.log(
					`Skipping duplicate actor rating for actor ${actor.id} by user ${user.id}`,
				);
			}
		}
	}
	...
	for (const crewMember of crew) {
		...
		if (Math.random() > 0.3) {
			...
			try {
				...
				console.log(
					`Skipping duplicate crew review for crew ${crewMember.id} by user ${user.id}`,
				);
			}
		}
	}
...
export async function generateDynamicSeedData(
	startFromStep: SeedStep = SeedStep.Movies,
) {

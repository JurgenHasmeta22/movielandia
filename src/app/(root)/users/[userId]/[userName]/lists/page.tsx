...
export default async function ListsPage(props: PageProps) {
	...
	const isOwnProfile =
		session?.user?.id && Number(session.user.id) === userId;
	...
	const displayLists =
		view === "shared" && isOwnProfile ? sharedLists : lists;
	const displayListsCount =
		view === "shared" && isOwnProfile ? sharedListsCount : listsCount;

...
export default function ForumSearchResultsList({
	...
	if (tabValue === 0) {
		const allItems = [
			...searchResults.topics.items.map((item) => ({
				...item,
				type: "topic",
			})),
			...searchResults.posts.items.map((item) => ({
				...item,
				type: "post",
			})),
			...searchResults.replies.items.map((item) => ({
				...item,
				type: "reply",
			})),
		];
		...
		items = searchResults.topics.items.map((item) => ({
			...item,
			type: "topic",
		}));
		total = searchResults.topics.total;
	} else if (tabValue === 2) {
		items = searchResults.posts.items.map((item) => ({
			...item,
			type: "post",
		}));
		total = searchResults.posts.total;
	} else if (tabValue === 3) {
		items = searchResults.replies.items.map((item) => ({
			...item,
			type: "reply",
		}));

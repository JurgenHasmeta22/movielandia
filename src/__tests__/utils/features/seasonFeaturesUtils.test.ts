...
describe('Season Features Utils', () => {
  ...
  describe('onBookmarkSeason', () => {
    it('should add season to favorites and show success toast', async () => {
      ...
      expect(addFavoriteSeasonToUser).toHaveBeenCalledWith(1, mockSeason.id);
      expect(showToast).toHaveBeenCalledWith('success', 'Season added to favorites!');
    });
    ...
    it('should handle unknown errors', async () => {
      vi.mocked(addFavoriteSeasonToUser).mockRejectedValueOnce('Unknown error');
      
      await onBookmarkSeason(mockSession, mockSeason);
      
      expect(addFavoriteSeasonToUser).toHaveBeenCalledWith(1, mockSeason.id);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while adding the season to favorites.');
      expect(console.error).toHaveBeenCalled();
    });
  ...
  describe('onRemoveBookmarkSeason', () => {
    it('should remove season from favorites and show success toast', async () => {
      ...
      expect(removeFavoriteSeasonToUser).toHaveBeenCalledWith(1, mockSeason.id, `/seasons/${mockSeason.title}`);
      expect(showToast).toHaveBeenCalledWith('success', 'Season removed from favorites!');
    });

    it('should handle errors and show error toast', async () => {
      const testError = new Error('Test error');
      vi.mocked(removeFavoriteSeasonToUser).mockRejectedValueOnce(testError);
    ...
    it('should handle unknown errors', async () => {
      vi.mocked(removeFavoriteSeasonToUser).mockRejectedValueOnce('Unknown error');
      
      await onRemoveBookmarkSeason(mockSession, mockSeason);
      
      expect(removeFavoriteSeasonToUser).toHaveBeenCalledWith(1, mockSeason.id, `/seasons/${mockSeason.title}`);
      expect(showToast).toHaveBeenCalledWith('error', 'An unexpected error occurred while removing the season from favorites.');
      expect(console.error).toHaveBeenCalled();
    });

    it('should do nothing if session or season is missing', async () => {
      await onRemoveBookmarkSeason(null as unknown as Session, mockSeason);
      expect(removeFavoriteSeasonToUser).not.toHaveBeenCalled();
      
      await onRemoveBookmarkSeason(mockSession, null as unknown as Season);
      expect(removeFavoriteSeasonToUser).not.toHaveBeenCalled();

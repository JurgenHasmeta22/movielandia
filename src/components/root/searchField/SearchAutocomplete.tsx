...
const SearchAutocomplete = ({
	...
	const InitialState = () => (
		...
		>
			<Typography variant="body1">
				Select a category and start typing to search
			</Typography>
			<Typography variant="caption">
	...
	const getFilterSpecificResults = () => {
		...
		const counts = selectedCategories
			.map((filter) => {
				const key =
					filter.value === "crew" ? "crews" : `${filter.value}`;
				const category =
					results[key.toLowerCase() as keyof typeof results];

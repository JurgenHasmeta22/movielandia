...
describe("Actor Features Utils", () => {
	...
	describe("onBookmarkActor", () => {
		it("should add actor to favorites and show success toast", async () => {
			...
			expect(addFavoriteActorToUser).toHaveBeenCalledWith(
				1,
				mockActor.id,
			);
		...
		it("should handle unknown errors", async () => {
			vi.mocked(addFavoriteActorToUser).mockRejectedValueOnce(
				"Unknown error",
			);
			...
			expect(showToast).toHaveBeenCalledWith(
				"error",
				"An unexpected error occurred while adding the actor to favorites.",
			);
			expect(console.error).toHaveBeenCalled();
		});
	...
	describe("onRemoveBookmarkActor", () => {
		it("should remove actor from favorites and show success toast", async () => {
			...
			expect(removeFavoriteActorToUser).toHaveBeenCalledWith(
				1,
				mockActor.id,
			);
			expect(showToast).toHaveBeenCalledWith(
				"success",
				"Actor removed from favorites!",
			);
		});

		it("should handle errors and show error toast", async () => {
			const testError = new Error("Test error");
			vi.mocked(removeFavoriteActorToUser).mockRejectedValueOnce(
				testError,
			);
		...
		it("should handle unknown errors", async () => {
			...
			expect(showToast).toHaveBeenCalledWith(
				"error",
				"An unexpected error occurred while removing the actor from favorites.",
			);
			expect(console.error).toHaveBeenCalled();
		});
